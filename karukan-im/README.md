...
「きょう」などの読みを Space で変換すると、候補に今日の日付が現れます。対象の読みと日付オフセットは次のとおりです。
...
出力フォーマットは `[date] formats` で設定します（[chrono](https://docs.rs/chrono/) の strftime 記法）。複数指定するとそれぞれが別の候補になります。既定は `%Y-%m-%d`（例 `2026-07-13`）と `%Y年%-m月%-d日`（例 `2026年7月13日`）です。`[date] enabled = false` で無効化できます。
