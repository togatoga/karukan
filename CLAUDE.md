...
- `core/engine/` — IMEEngine state machine (Empty → Composing → Conversion)
  ...
  - `chunk/split.rs` — Where the chunk boundaries fall, engine-free: `tokenize` classifies every char into a `Token`, `group_chunks` pops them off the queue and packs them into chunks under `ChunkLimits` (only a number is taken as a unit, via `digit_run`). Pure functions, unit-tested in place
