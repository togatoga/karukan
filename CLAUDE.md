...
- Config hot-reload: `reload_config_if_changed` (`core/engine/reload.rs`) re-reads config.toml when its mtime changed and swaps in the `EngineConfig` subset (persona, chunking, beam/strategy, context lengths; `live_conversion` only on value change so the runtime Ctrl+Shift+L toggle survives); model/dict/n_threads/[learning] changes are logged as restart-required. No file watching (matches fcitx5/mozc/azooKey) — triggers are the fcitx5 addon's `activate()`/`reloadConfig()` (the latter also fired via a `GlobalConfigReloadedEvent` watch so `fcitx5-remote -r` works) and the macOS `reload_config` RPC sent from `activateServer`
- Conversion persona: `[conversion] persona` in config.toml (free keywords, e.g. 「プログラミング」) is prepended to every model lctx by bare concatenation (`{persona}{ctx}`) inside `run_kana_kanji_conversion` (`persona_lctx`; last 25 chars via `effective_persona`, empty = off), so live chunks and Space conversion both carry it and it participates in the conversion cache key; the aux mode indicator shows the effective content (e.g. ⚡[あ]P:プログラミング). The config key is `persona` because "profile" is reserved for planned per-user profile switching (separate model/learning cache/user dicts/persona per profile)
