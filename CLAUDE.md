...
- Learning cache records user-selected conversions and boosts them on subsequent conversions; candidate priority: Learning → User Dictionary → Model → System Dictionary → Fallback → Rewriter. Surfaces longer than `max_surface_chars` (default 50, `[learning]` in config.toml) are not recorded. During conversion, Ctrl+Delete removes the selected learning candidate from the history, mozc-style (`DeleteSelectedCandidate`): exact reading+surface removal, then the conversion is cancelled back to composing; while a learning candidate is selected the aux text shows the mozc footer hint 「Ctrl+Delで履歴から削除」
