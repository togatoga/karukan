...
- The greedy generation path (`LlamaCppModel::generate`) keeps a persistent context per model and re-decodes only the prompt suffix past the longest common token prefix with the previous call (`GreedySession`), so live typing pays a few tokens per keystroke instead of a full prefill (measured 2.2× faster per keystroke). Output is identical to a fresh context by construction and by test; beam paths and `NllScorer` are untouched
