...
| Ctrl+R / Ctrl+T | 変換を開始してソース絞り込みビューへ（Ctrl+R は📝学習から、Ctrl+T は🔄記号変換から） |
| Ctrl+Space | 全角スペースを入力 |

未入力の状態で `:` を打つと絵文字モードになり、`:smile` のようなクエリで絵文字を検索できます。Enterで先頭の絵文字候補を確定、Escapeで入力した文字列をそのまま確定します。

`Ctrl+1〜9`・`Ctrl+J`・`Ctrl+I`・`Ctrl+R` / `Ctrl+T` は変換モードでも使えます。どちらのモードでも「いま画面に出ているもの」に対して働きます。
...
| Ctrl+R / Ctrl+T （次 / 前） | 候補をソース別に絞り込み（📝学習→📚辞書→🤖AI→🔄記号変換をローテーション。📚はユーザー辞書とシステム辞書の両方で、ユーザー辞書が先。🔄の末尾に素のひらがな/カタカナも並ぶ。全体に戻るには Esc→Space） |
| 文字キー・数字 | 確定せず読みを伸ばして変換をやり直す（インクリメンタルに候補が更新される。ソース絞り込み中はフィルタを維持したままビュー内で候補が絞られる） |
